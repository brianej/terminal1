...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def start_strategy_1(self, game_state):
        ...
        if game_state.turn_number % 4 == 0 and game_state.turn_number > 10:
            if game_state.turn_number >= 20:
                game_state.attempt_spawn(DEMOLISHER, [17, 3], 1000)
            else:
                if random.randint(1, 4) in [1, 2, 3]:
                    game_state.attempt_spawn(SCOUT, [17, 3], 1000)
                else:
                    game_state.attempt_spawn(SCOUT, [17, 3], 1000)

        if game_state.turn_number in [10, 16, 21, 25]:
            self.send_interceptors(game_state, [26, 13])
    
    def send_interceptors(self, game_state, preferred_location):
        if game_state.contains_stationary_unit(preferred_location):
            # Find a nearby location that is not occupied
            nearby_locations = [[25, 13], [26, 12], [27, 13], [26, 14], [25, 12]]
            for loc in nearby_locations:
                if not game_state.contains_stationary_unit(loc):
                    game_state.attempt_spawn(INTERCEPTOR, loc, 1)
                    return
        else:
            game_state.attempt_spawn(INTERCEPTOR, preferred_location, 1)
    ...
    def build_defences(self, game_state):
        ...
        # Build and upgrade more supports earlier
        more_support_locations = [[18, 6], [17, 6], [16, 6], [15, 6], [14, 6], [13, 6]]
        game_state.attempt_spawn(SUPPORT, more_support_locations)
        game_state.attempt_upgrade(more_support_locations)

        game_state.attempt_upgrade(support_locations)
        game_state.attempt_spawn(SUPPORT, support_locations)

        # Upgrade the four turrets on the very right
        right_turrets = [[24, 13], [27, 13], [26, 12], [25, 11]]
        game_state.attempt_upgrade(right_turrets)

        # Do not upgrade other turrets unless specified
